...
let createStyleHash = styles => {
  let _table = Hashtbl.create(100);
  List.iter(
    i => {
      let styleValue =
        switch (i) {
        ...
        | `Margin2(m2) => ("margin2", m2)
        | `Margin4(m4) => ("margin4", m4)
      ...
      Hashtbl.add(_table, styleValue, true);
    },
    styles,
  );

  Hashtbl.find_opt(_table, ())
};
