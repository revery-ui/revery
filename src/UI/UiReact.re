...
module Container = {
    ...
    let createContainer: UiReconciler.reveryNode => t = (n) => {
        node: n,
        state: None,
    };

    let updateContainer: (t, React.syntheticElement) => t = ({node, state}, element) => {
        let newRendered = switch (state) {
        | None => {
        ...
        | Some(s) => {
            let nextElement = React.RenderedElement.update(~previousElement=s.previousElement, ~renderedElement=s.rendered, element)
