...
let init = app => {
  ...
  let hourHandStyle = clock =>
    baseStyle(~width=16, ~height=50, ~transform=[RotateZ(Angle.from_degrees(float(clock.Unix.tm_hour mod 12) *. 30.0)), TranslateY(0.)], ());

  let minuteHandStyle = clock =>
    baseStyle(~width=8, ~height=75, ~transform=[RotateZ(Angle.from_degrees(float(clock.Unix.tm_min) *. 6.0)), TranslateY(0.)], ());

  let secondHandStyle = clock =>
    baseStyle(~width=4, ~height=100, ~transform=[RotateZ(Angle.from_degrees(float(clock.Unix.tm_sec) *. 6.0)), TranslateY(0.)], ());
