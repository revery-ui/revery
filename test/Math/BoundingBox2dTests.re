...
test("BoundingBox2d", () => {
  ...
  test("intersection", () => {
	let bbox1 = BoundingBox2d.create(Vec2.create(5., 5.), Vec2.create(10., 10.));
	let bbox2 = BoundingBox2d.create(Vec2.create(0., 0.), Vec2.create(4., 4.));

	expect(BoundingBox2d.intersects(bbox1, bbox2)).toBe(false);

	let leftIntersect = BoundingBox2d.create(Vec2.create(4., 4.), Vec2.create(6., 6.));

	expect(BoundingBox2d.intersects(bbox1, leftIntersect)).toBe(true);

	let topIntersect = BoundingBox2d.create(Vec2.create(6., 4.), Vec2.create(8., 6.));

	expect(BoundingBox2d.intersects(bbox1, topIntersect)).toBe(true);

	let rightIntersect = BoundingBox2d.create(Vec2.create(7., 6.), Vec2.create(11., 8.));

	expect(BoundingBox2d.intersects(bbox1, rightIntersect)).toBe(true);

	let bottomIntersect = BoundingBox2d.create(Vec2.create(6., 9.), Vec2.create(7., 11.));
